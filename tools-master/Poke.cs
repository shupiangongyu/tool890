...
namespace Tools
{
    public class PokeImg
    {
		...
		*/
        ...
        public Bitmap iconImportedImg;
        public bool iconImported;
        public int palindex;
        ...
        {
            iconImported = false;
            iconImportedImg = null;
            rw.Seek(icoinPalleteIndexTable + index);
            palindex = rw.Br.ReadByte();
        ...
        public void SaveAll(ReadAndWrite rw, bool fill)
        {
            ...
            {
                rw.Seek(icoinPalleteIndexTable + index);
                rw.Bw.Write((byte)palindex);
                var byteData = ImgFunction.ConvertNormalImagToGBA(iconImportedImg, new List<Color>(icoinPalltes[palindex]), false);
                var offset_old = rw.ReadPointer(icoinTable + index * 4);
                if (!fill)
                {
                    Save(rw, byteData, icoinTable + index * 4, false);
                }
                else
                {
                    rw.WriteBytes(offset_old, 0, byteData);
                }
        ...
        public Bitmap ImportIcon(Bitmap img)
        {
            iconImported = true;
            iconImportedImg = img;
            return  ImgFunction.ConvertToBitmapIndexed(img, new List<Color>(icoinPalltes[palindex]));
        }

        public Bitmap ChangeIconClr(int frames)
        {
            if (iconImportedImg == null) return null;
            palindex = frames;
            return ImgFunction.ConvertToBitmapIndexed(iconImportedImg, new List<Color>(icoinPalltes[palindex]));
        }
    }
