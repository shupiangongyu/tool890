...
namespace Tools {
	public class PokeConfig : ConfigParser {
		...
		private PokeConfig(string path) : base(path) {
			section = Get("global", "section", "802");
		}
		public static INI_init ini;

		public static PokeConfig of(string path)
		{
			if (File.Exists(path))
			{
				return new PokeConfig(path);
			}
			return null;
		}

		public static void Validate(ReadAndWrite rw) {
            /*
			rw.Seek(0xAC);
