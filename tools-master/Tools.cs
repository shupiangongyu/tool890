...
namespace Tools
{
    ...
    public partial class MainWindow : Form
    {
        ...
        public MainWindow()
        {
            ...
            evoPokeNames.SelectedIndexChanged += evolution.setPokeIndex;
        ...
        public void SetAction()
        {
            pokeName.SelectedIndexChanged +=
                pokeName_SelectedIndexChanged;
            listBox_map.SelectedIndexChanged +=
                listBox_map_SelectedIndexChanged;
            button1.Click += button1_Click;
            wildmaptype.SelectedIndexChanged +=
                wildmaptype_SelectedIndexChanged;
            comboBox_item_no.SelectedIndexChanged +=
                comboBox_item_no_SelectedIndexChanged;
            button2.Click += button2_Click;
            learn_move.SelectedIndexChanged +=
                learn_move_SelectedIndexChanged;
            UpDown.ValueChanged +=
                UpDown_ValueChanged;
            bf_nb.ValueChanged +=
                nb_ValueChanged;
            trainer_num.SelectedIndexChanged +=
                trainer_num_ValueChanged;
            bf_items.SelectedIndexChanged +=
                bf_items_SelectedIndexChanged;
            PlayerY.ValueChanged +=
                pokePos.ChangePlayerY;
            Altitude.ValueChanged +=
                pokePos.ChangeEnemyAlt;
            EnemyY.ValueChanged +=
                pokePos.ChangeEnemyY;
        }
        ...
        void SetItemList()
        {
            ...
            bf_itemlist.SelectedIndexChanged -=
                bf_itemlist_SelectedIndexChanged;
        ...
        void Save_basestats()
        {
            using (ReadAndWrite rw = new ReadAndWrite(rom_path)) {
                ...
                int evs = 0;
                evs |= (byte.Parse(evs_spdef.Text) > 3 ?
        ...
        private void ow_Click(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left) {
                ...
                i = ImgFunction.ConvertToBitmapIndexed(i);
        ...
        private void button12_Click_1(object sender, EventArgs e)
        {
            ...
            if (img.Height == 32)
            {
                Bitmap bm = new Bitmap(32, 64);
                using var g = Graphics.FromImage(bm);
                g.DrawImage(img, new Rectangle(0, 0, 32, 32));
                g.DrawImage(img, new Rectangle(0, 32, 32, 32));
                img = bm;

            }
            iconbox.Image =  pokeimg.ImportIcon(img);
        }
        ...
        private void iconPal_ValueChanged(object sender, UpDownEventArgs e)
        {
            var n = (NumericUpDown)sender;
            
            var img = pokeimg.ChangeIconClr((int)n.Value);
            if (img != null)
            {
                iconbox.Image = img;
            }
